...
{
    printf("DISPATCH KEY key: %u, time: %u, state: %d\n", key, time, state);
...
{
    printf("START REPEATING KEY key: %u, time: %u\n", key, time);
...
static void stop_repeating_key()
{
    printf("STOP REPEATING KEY\n");
...
key_repeat_source_dispatch (GSource *base, GSourceFunc callback, gpointer user_data)
{
    ...
    input_dispatch_key_event(input_data.repeating_key.time, input_data.repeating_key.key, LIBINPUT_KEY_STATE_PRESSED);
